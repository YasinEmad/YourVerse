...
| `catalog.ts` | Catalog endpoints: `getWorlds`, `getWorld`, `getWorldProducts`, `getProduct`. Used by the world/product pages (built in Phase 1-2). |
...
- `hooks/useCart.ts` — the only public surface components use; hydrates on mount. Optimistic mutations automatically reconcile with the server response once it arrives, and roll back to the last confirmed state if the request fails.

### 3. Shared order totals (`lib/orders/order-totals.ts`)

- Single source of truth for money math: `computeOrderTotals` plus constants `SHIPPING_FLAT_RATE = 5` and `TAX_RATE = 0.1`.
- Used everywhere money is derived — the cart store, `OrderSummary`, the mock order route, and the server-rendered confirmation page — so checkout and confirmation always apply the same calculation and business rules cannot drift.

### 4. Checkout flow (`components/shop/`)
...
- `cart/page.tsx` (client), `checkout/page.tsx` (server shell), `checkout/confirmation/[orderId]/page.tsx` (server-rendered). The confirmation page does **not** trust client-provided totals: it loads the order from the server by id and re-derives every total via `computeOrderTotals(order.items)` before rendering, so anything shown reflects the server's data.
...
- Smoke test against `npm run dev`:
  ...
  - Guest session cookie (`yourverse-session`) persists the cart across a full page reload — the store rehydrates from the server using the same session, so a reload does not lose the cart (satisfies the Definition of Done for persistence).
...
- **Adding a checkout step** requires only a new step component plus one entry in the `checkoutSteps` configuration array — the sequence is data-driven and the wrapper renders the active step from that array.
- **Adding a payment method** requires only one descriptor in `payment-methods.tsx` (with its `requiredFields`) and one payment form component registered alongside the existing forms — validation and rendering are driven by the descriptor, so no checkout code changes.
- **Replacing the cart implementation** (for example, swapping the store for Zustand) should require changes only inside the cart store and `useCart()`: components depend solely on the `useCart()` public surface, so the rest of the shop remains untouched.
